...
function main(){
    ...
    info "Restarting docker container \"${APOLLO_DEV}\" ..."
    ...
    ${DOCKER_CMD} start $APOLLO_DEV

    set +x
    if [ $? -ne 0 ];then
        error "Failed to restart docker container \"${APOLLO_DEV}\""
        exit 1
    fi

    ok "Finished restarting Apollo docker environment. Now you can enter with: \nbash docker/scripts/dev_into.sh"
    ok "Enjoy!"
}
